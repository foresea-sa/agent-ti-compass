...
HELPER_SCRIPTS=(
  ...
  test-login-linux.sh
  validate-secrets-linux.sh
  enable-services-linux.sh
...
for helper in "${HELPER_SCRIPTS[@]}"; do
  ...
  install -m 0755 -o root -g root "$SOURCE_DIR/$helper" "$INSTALL_DIR/$helper"
done

# O configurador Python precisa existir porque configure-credentials.sh o chama.
[[ -f "$SOURCE_DIR/credential_setup.py" ]] || fail "Arquivo obrigatorio ausente: credential_setup.py"
install -m 0644 -o root -g root "$SOURCE_DIR/credential_setup.py" "$INSTALL_DIR/credential_setup.py"
