...
class CompassCollector:
    ...
    def _login(self, page) -> None:
        ...
        if user_field is None:
            raise RuntimeError("Campo de usuario/e-mail nao encontrado na tela de login do Compass.")
        user_field.fill(username)

        # O portal mostrado nos prints usa fluxo em duas etapas: e-mail -> Login -> senha.
        ...
        while time.time() < deadline:
            ...
            if password_field is not None:
                break
            # Alguns fluxos podem concluir o login na primeira etapa quando ha sessao previa.
        ...
        if password_field is None:
            raise RuntimeError("Campo de senha nao apareceu apos o envio do usuario no Compass.")
        ...
        raise RuntimeError("O Compass permaneceu na tela de login apos o envio de usuario e senha.")

    def _failure_screenshot(self, page, name: str) -> Path:
        stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        path = DEBUG_DIR / f"{stamp}_{name}.png"
        try:
    ...
    def _session_expired(self, page) -> bool:
        if "/login" in page.url.lower():
            return True
        try:
            return page.get_by_text("Welcome to Compass", exact=False).count() > 0
        except Exception:
            return False
    ...
    def collect(self):
        ...
        with sync_playwright() as p:
            ...
            if cfg.get("use_saved_session", True) and state_path.exists():
            ...
            try:
                page.get_by_text("Starlink Fleet Usage", exact=False).first.wait_for(
                    state="visible", timeout=cfg.get("timeout_ms", 60000)
                )
            except Exception as exc:
                shot = self._failure_screenshot(page, "usage_page_not_ready")
                browser.close()
                raise RuntimeError(f"Pagina Starlink Fleet Usage nao ficou pronta. Screenshot: {shot}. Erro: {exc}")
